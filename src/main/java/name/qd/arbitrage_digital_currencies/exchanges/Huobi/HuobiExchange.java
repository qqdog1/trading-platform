package name.qd.arbitrage_digital_currencies.exchanges.Huobi;
...
import name.qd.arbitrage_digital_currencies.exchanges.ExchangeConfig;
...
public class HuobiExchange implements Exchange {
	...
	public List<String> getProducts() {
		...
		try {
			...
			urlBuilder.addPathSegments("v1/common/symbols");
			String result = sendSyncHttpGet(urlBuilder.build().url().toString());
			JsonNode node = objectMapper.readTree(result);
			JsonNode dataNode = node.get("data");
			for(JsonNode nodeProduct : dataNode) {
				String base = nodeProduct.get("base-currency").asText();
				String quote = nodeProduct.get("quote-currency").asText();
				lst.add(base+quote);
			}
		} catch (IOException e) {
	...
	public List<Double> getInstantPrice(String product) {
		...
		try {
			HttpUrl.Builder urlBuilder = httpUrl.newBuilder();
			urlBuilder.addPathSegments("market/depth");
			urlBuilder.addEncodedQueryParameter("symbol", product);
			urlBuilder.addEncodedQueryParameter("type", "step0");
			String result = sendSyncHttpGet(urlBuilder.build().url().toString());
			JsonNode node = objectMapper.readTree(result);
			JsonNode dataNode = node.get("tick");
			JsonNode bidNode = dataNode.get("bids").get(0);
			JsonNode askNode = dataNode.get("asks").get(0);
